...
private:
  ...
  void publishOccupancyGrid(
    const std::shared_ptr<const glidar_slam::mapping::GlobalMapSnapshot> & snapshot,
  ...
  static sensor_msgs::msg::Image::UniquePtr makeGroundDebugImage(
    const cv::Mat & color, const glidar_slam::CameraIntrinsics & intrinsics,
    const Eigen::Affine3f & base_from_camera, const std::vector<float> & roi_ratios,
    const glidar_slam::GroundPlaneObservation & observation, const rclcpp::Time & stamp,
