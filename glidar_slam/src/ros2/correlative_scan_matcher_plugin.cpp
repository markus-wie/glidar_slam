#include "glidar_slam/ros2/correlative_scan_matcher_plugin.hpp"
...
void CorrelativeScanMatcherPlugin::initialize(rclcpp::Node * node, const std::string & name)
{
  ...
  auto parameters = std::make_shared<core::Parameters>();
  ...
  matcher_ = std::make_unique<core::CorrelativeScanMatcher>(parameters);
}

core::CsmResult CorrelativeScanMatcherPlugin::match(
  const core::SubmapGrid & submap_grid, const std::vector<core::Point2D> & current_points,
  const core::Pose2D & pose_estimate) const
{
  if (!matcher_) {
...
PLUGINLIB_EXPORT_CLASS(
  glidar_slam::ros2::CorrelativeScanMatcherPlugin, glidar_slam::ros2::ScanMatcherInterface)
