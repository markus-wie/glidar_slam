...
#include "glidar_slam/ground_plane_factor.hpp"
...
public:
  ...
  void addPriorFactor(uint64_t key, const gtsam::Pose3 & pose);
  void addPriorFactor(uint64_t key, const gtsam::Pose3 & pose, const gtsam::Matrix66 & covariance);
  ...
  std::vector<SerializedFactor> getSerializedFactors() const;
  uint64_t getLatestKey() const;
  uint64_t getLatestTimestamp() const;

  bool restore(const SlamStateSnapshot & snapshot);
...
private:
  ...
  // std::vector<SerializedFactor> serialized_factors_;

  mutable std::mutex isam_mutex_;
};
